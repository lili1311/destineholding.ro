...
export default function AdminPage() {
  ...
  useEffect(() => {
    // Încarcă lead-urile din localStorage
    const savedLeads = JSON.parse(localStorage.getItem("destine-leads") || "[]")
    setLeads(savedLeads)
  }, [])
  ...
  const handleStatusChange = (leadId: string, newStatus: string) => {
    const updatedLeads = leads.map((lead) => (lead.id === leadId ? { ...lead, status: newStatus as any } : lead))
    setLeads(updatedLeads)
    localStorage.setItem("destine-leads", JSON.stringify(updatedLeads))
  }
  ...
  const stats = {
    total: leads.length,
    nou: leads.filter((l) => l.status === "nou").length,
    contactat: leads.filter((l) => l.status === "contactat").length,
    vandut: leads.filter((l) => l.status === "vandut").length,
    refuzat: leads.filter((l) => l.status === "refuzat").length,
    astazi: leads.filter((l) => new Date(l.created_at).toDateString() === new Date().toDateString()).length,
  }
      ...
      <div className="mb-8">
        ...
        <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="flex items-start">
            ...
            <div>
              <h3 className="text-sm font-medium text-green-800">Sistem Funcțional</h3>
              <p className="text-sm text-green-700 mt-1">
                Lead-urile se salvează în browser (localStorage). Pentru salvare permanentă și email-uri automate,
                configurează Supabase și Resend.
              </p>
            </div>
          </div>
        </div>
